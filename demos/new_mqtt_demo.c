...
void *demo_mqtt_send_thread(void *args)
{
    int32_t res = STATE_SUCCESS;
    char topic[128] = {0};
    const char *hex_data = "010300000001840A";
    ...
    /* 转换十六进制字符串为二进制数据 */
    binary_len = hex_to_binary(hex_data, binary_data, sizeof(binary_data));
    
    while (g_mqtt_send_thread_running) {
        res = aiot_mqtt_pub(args, topic, binary_data, binary_len, 0);
        if (res < 0) {
